...
import { applyEventStatImport, previewEventStatImport } from "../lib/sports/stat-ingestion";

async function main() {
  const prisma = new PrismaClient();
  const apply = process.argv.includes("--apply");
  const pathArg = process.argv.find((arg) => arg.endsWith(".json"));
  if (!pathArg) throw new Error("Provide an event-stat JSON file path");
  const raw = await readFile(resolve(pathArg), "utf8");
  const preview = await previewEventStatImport(raw, null, prisma);
  ...
  if (apply) {
    const result = await applyEventStatImport(preview.id, null, prisma);
