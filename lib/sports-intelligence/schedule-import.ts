...
export async function applyScheduleRun(prisma: PrismaClient, runId: string) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    for (const item of pkg.teams) {
      const team = await tx.team.upsert({ where: { key: item.key }, create: { key: item.key, name: item.name, marketId: markets.get(item.marketKey)!.id, sport: pkg.sport.name, league: pkg.league.code, leagueId: league.id, subdivision: item.subdivision, abbreviation: item.abbreviation, homeVenueId: item.homeVenueKey ? venues.get(item.homeVenueKey)?.id : null }, update: { name: item.name, marketId: markets.get(item.marketKey)!.id, sport: pkg.sport.name, league: pkg.league.code, leagueId: league.id, subdivision: item.subdivision, abbreviation: item.abbreviation, homeVenueId: item.homeVenueKey ? venues.get(item.homeVenueKey)?.id : null } });
