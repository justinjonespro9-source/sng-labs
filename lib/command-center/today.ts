...
export type TodayItem = {
  id: string;
  sourceType: "GROWTH_EVENT" | "ACTIVATION" | "CAMPAIGN" | "CONTENT_DRAFT" | "PUBLICATION" | "RELATIONSHIP_FOLLOW_UP" | "SOCIAL_ACCOUNT_HEALTH";
  sourceId: string;
  title: string;
  startsAt: string;
  endsAt?: string;
...
export function dedupeOpenSocialHealth<T extends { dedupeKey: string; observedAt: Date }>(items: T[]) {
  const byKey = new Map<string, T>();
  for (const item of items) {
    const current = byKey.get(item.dedupeKey);
    if (!current || item.observedAt > current.observedAt) byKey.set(item.dedupeKey, item);
  }
  return [...byKey.values()];
}
...
export async function getTodayReadModel(prisma: PrismaClient, input: { from: Date; to: Date }): Promise<TodayItem[]> {
  ...
  const [events, activations, campaigns, drafts, publications, relationships, socialHealth] = await Promise.all([
    ...
    prisma.socialConnectionHealthObservation.findMany({ where: { operatorActionRequired: true, resolvedAt: null, observedAt: { lt: input.to } }, include: { socialAccount: { include: { brand: true } } }, orderBy: { observedAt: "desc" }, distinct: ["dedupeKey"] }),
  ]);
  return orderTodayItems([
    ...
    ...dedupeOpenSocialHealth(socialHealth).map((item): TodayItem => ({ id: `SOCIAL_ACCOUNT_HEALTH:${item.id}`, sourceType: "SOCIAL_ACCOUNT_HEALTH", sourceId: item.id, title: `${item.socialAccount.brand.name} ${item.socialAccount.platform}: ${item.summary}`, startsAt: (item.observedAt < input.from ? input.from : item.observedAt).toISOString(), status: item.state, href: "/command-center/account-health", lenses: ["DISTRIBUTION", "PORTFOLIO"] })),
  ]);
}
