...
export async function resolveGenerationContext(input: GenerationInput) {
  const [brand, selectedProgram, selectedCampaign, selectedActivation, selectedOpportunity, relationship] = await Promise.all([
    ...
    input.opportunityId ? prisma.opportunity.findUnique({ where: { id: input.opportunityId }, include: { activation: { include: activationInclude }, campaigns: { include: { campaign: { include: campaignInclude } } }, markets: { select: { id: true, name: true } }, teams: { select: { id: true, name: true } }, angles: { include: { brand: { select: { id: true, name: true } } } } } }) : null,
